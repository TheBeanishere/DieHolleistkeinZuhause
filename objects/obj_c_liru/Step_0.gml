if (obj_game.AI_liru > 0 && obj_office.powerleft > 0){
	...
	if (stage < 5 && instance_exists(obj_camerabutton)){
		movetimer -= 1
		if (movetimer <= 0){
			movetimer = 360
			randomize()
			...
			if (_move <= obj_game.AI_liru){
				if (stage < 4){
					...
					audio_stop_sound(sfx_liru_1)
					audio_stop_sound(sfx_liru_2)
					audio_stop_sound(sfx_liru_3)
					if (global.camera = 2){
						scr_camerastatic()
					}
					if (global.cameraflipped){
						...
						obj_o_liru.sprite_index = spr_office_liru_down
					}
